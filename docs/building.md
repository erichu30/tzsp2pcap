...
need to be installed on the host. Each command writes the requested binary
directly under `dist/`:
...
`dist/tzsp2pcap-linux-amd64` and `dist/tzsp2pcap-linux-arm64`. Build outputs are
ignored by Git. Only arm64 verifies the `-R BYTES` help text and executes the binary
...
and retain the binaries as workflow artifacts. Verification is part of the
...
- `tzsp2pcap-linux-amd64`
- `tzsp2pcap-linux-arm64`
- `checksums.txt`
